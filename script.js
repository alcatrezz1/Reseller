...
(function () {
  ...
  // Prepend clones of last CLONE_N originals
  for (let i = N - CLONE_N; i < N; i++) {
    const c = origItems[i].cloneNode(true);
    c.setAttribute('aria-hidden', 'true');
    track.insertBefore(c, track.firstChild);
  }
  // Append clones of first CLONE_N originals
  ...
  function updateDots() {
    ...
    if (!el) return;
    const vc = visCount();
    const realIdx = ((cur - CLONE_N) % N + N) % N;
    const page = Math.floor(realIdx / vc);
    const pages = Math.ceil(N / vc);
    el.innerHTML = '';
    for (let i = 0; i < pages; i++) {
      const b = document.createElement('button');
      b.className = 'dot' + (i === page ? ' active' : '');
      b.addEventListener('click', () => { cur = CLONE_N + i * vc; setPos(true); updateDots(); startAuto(); });
      el.appendChild(b);
    }
  }
  ...
  track.addEventListener('transitionend', e => {
    ...
    if (cur >= CLONE_N + N) { cur -= N; setPos(false); }
    else if (cur < CLONE_N)  { cur += N; setPos(false); }
    updateDots();
  });

  function startAuto() {
    clearInterval(autoT);
    autoT = setInterval(() => goTo(cur + 1), 4000);
  }
